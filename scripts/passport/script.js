...
let current_type = -1;
let elements_name = "block_";
let number_of_elements = 11;

document.addEventListener("DOMContentLoaded", () => {
    document.getElementById("edit").addEventListener("click", (event) => {
        for (let i = 1; i <= number_of_elements; i++) {
            let current_element = document.getElementById(elements_name + i);
            let string = current_element.innerHTML;

            string = translit(string, current_type * (-1) * 5);

            console.log(string);
            current_element.innerHTML = string;

            if (current_type == -1) {
                document.getElementById("edit").innerHTML = "Сменить на кириллицу";
            } else {
                document.getElementById("edit").innerHTML = "Сменить на латиницу";
            }
        }
